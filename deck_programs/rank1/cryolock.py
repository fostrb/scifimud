...
class Cryolock(DeckProgram):
    def __init__(self, name="cryolock"):
        super(Cryolock, self).__init__()
        self.name = name
        self.parser = argparse.ArgumentParser()
        self.parser.add_argument('-u', '--unlock', action='store')
        self.parser.add_argument('-l', '--lock', action='store')
...
class Minesweeper(object):
    ...
    def corrected_board(self):
        for r in range(self.rows):
            for c in range(self.cols):
                if self.viewable_board[r][c] == FLAG and self.values[r][c] != 'x':
    ...
    def guess_surrounding(self, r, c):
        ...
        if type(self.viewable_board[r][c]) is not int:
            print('You may only use the sweep function on boxes you have uncovered')
            return
        rows = self.rows
        cols = self.cols
        if (r > 0 and c > 0 and self.viewable_board[r-1][c-1] == BLOCK):
            self.viewable_board[r-1][c-1] = self.values[r-1][c-1]
            if self.values[r-1][c-1] is 0:
                self.guess_surrounding(r-1, c-1)
        if (r > 0 and self.viewable_board[r-1][c] == BLOCK):
            self.viewable_board[r-1][c] = self.values[r-1][c]
            if self.values[r-1][c] is 0:
                self.guess_surrounding(r-1, c)
        if (r > 0 and c < cols-1 and self.viewable_board[r-1][c+1] == BLOCK):
            self.viewable_board[r-1][c+1] = self.values[r-1][c+1]
            if self.values[r-1][c+1] is 0:
                self.guess_surrounding(r-1, c+1)
        if (c > 0 and self.viewable_board[r][c-1] == BLOCK):
            self.viewable_board[r][c-1] = self.values[r][c-1]
            if self.values[r][c-1] is 0:
                self.guess_surrounding(r, c-1)
        if (c < cols-1 and self.viewable_board[r][c+1] == BLOCK):
            self.viewable_board[r][c+1] = self.values[r][c+1]
            if self.values[r][c+1] is 0:
                self.guess_surrounding(r, c+1)
        if (r < rows-1 and c > 0 and self.viewable_board[r+1][c-1] == BLOCK):
            self.viewable_board[r+1][c-1] = self.values[r+1][c-1]
            if self.values[r+1][c-1] is 0:
                self.guess_surrounding(r+1, c-1)
        if (r < rows-1 and self.viewable_board[r+1][c] == BLOCK):
            self.viewable_board[r+1][c] = self.values[r+1][c]
            if self.values[r+1][c] is 0:
                self.guess_surrounding(r+1, c)
        if (r < rows-1 and c < cols-1 and self.viewable_board[r+1][c+1] == BLOCK):
            self.viewable_board[r+1][c+1] = self.values[r+1][c+1]
            if self.values[r+1][c+1] is 0:
                self.guess_surrounding(r+1, c+1)
    ...
    def won(self):
        for r in range(self.rows):
            for c in range(self.cols):
                if self.viewable_board[r][c] == BLOCK and self.values[r][c] != 'x':
                    return False
                elif self.viewable_board[r][c] == FLAG and self.values[r][c] != 'x':
                    return False
        return True
